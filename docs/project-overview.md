...
X축 바퀴는 셔틀 본체에 고정 장착되어 항상 X축 레일에 접지된 상태이다. Y축 바퀴는 수직 슬라이더에 장착되어 있으며, MG996R 서보 모터와 래크 앤 피니언 기구를 통해 위아래로 승강한다. Y축 바퀴가 하강하면 Y축 레일에 접지되어 Y 방향 이동이 가능하고, 상승하면 X축 바퀴만 접지되어 X 방향 이동 상태가 된다.

| Y축 슬라이더 상태 | 접지 바퀴 | 이동 가능 방향 |
| --- | --- | --- |
| 상승 (Y축 바퀴 올림) | X축 바퀴만 접지 | X축 (열 방향) 이동 |
| 하강 (Y축 바퀴 내림) | X축 + Y축 바퀴 접지 | Y축 (행 방향) 이동 |
...
| 5 | Y축 바퀴 상승(X축만 접지) → X축 목적 열까지 정밀 이동 | ESP32 #2 |
