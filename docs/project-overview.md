...
| ③ 전송 | MQTT (Wi-Fi) → Supabase | 분류 결과 → 백엔드 서버 전달 |
...
| ⑦ 기록 | Supabase | 그리드 위치(X·Y·Z)·Bin ID·수량·타임스탬프 저장 및 Realtime 발행 |
| ⑧ 시각화 | Unity 디지털 트윈 | Supabase Realtime 구독 → 3D 그리드 랙 실시간 렌더링 |

## 2.2 시스템 구성도

투입구(고정) 아래에 Pi Camera v2가 설치되어 있고, 부품을 촬영하면 RPi-Zero 2W가 Gemini API를 호출한다.

분류 결과는 MQTT로 백엔드 서버에 전달되고, 서버는 해당 부품이 보관될 그리드 위치(X·Y·Z 좌표)를 계산하여 4-way 셔틀에 이송 명령을 전송한다.
...
| 셔틀 방향 전환 | 크랭크 기구 (MG996R 서보 모터 + 임펠러) | X·Y 바퀴 교번 접지 |
...
MG996R 서보 모터 축에 고정된 임펠러의 양쪽 끝에 X축 바퀴와 Y축 바퀴가 각각 결합된 슬라이더가 링크로 연결되어 있다. 임펠러가 회전하면 X축 바퀴와 Y축 바퀴가 서로 번갈아 레일에 접지되며, 접지된 축 방향으로만 셔틀이 이동한다.

| 임펠러 상태 | 접지 바퀴 | 이동 가능 방향 |
| --- | --- | --- |
| -20° 회전 | X축 바퀴 접지 | X축 (열 방향) 이동 |
| 20° 회전 | Y축 바퀴 접지 | Y축 (행 방향) 이동 |

**구동 구성**

- 방향 전환: MG996R 서보 모터 + 크랭크 임펠러 (X·Y 바퀴 교번 접지)
...
| **구분** | **구동 방식** | **역할** |
| --- | --- | --- |
| 4-way 셔틀 방향 전환 | MG996R 서보 모터 + 크랭크 임펠러 | X·Y 바퀴 교번 접지로 이동 축 전환 |
...
| 5 | 셔틀 방향 전환(X축 접지) → X축 목적 열까지 정밀 이동 | ESP32 #2 |
| 6 | 셔틀 방향 전환(Y축 접지) → Y축 목적 행까지 정밀 이동 | ESP32 #2 |
| 7 | Z축 리프트 → 목적 층까지 수직 이동 → Bin 입고 확인 → 완료 신호 | ESP32 #2 |
| 8 | DB 업데이트 → Realtime 이벤트 발행 → Unity | 서버 |
...
| ESP32 #2 | 4-way 셔틀 전체 제어 (방향 전환 MG996R + MG90S X·Y축 + MG996R Z축 리프트) | Wi-Fi MQTT |
| Raspberry Pi #1 | Pi Camera v2 제어 + Gemini API 호출 + MQTT 브로커 | Wi-Fi MQTT/Ethernet |
...
Raspberry Pi의 경량 MQTT 브리지 스크립트가 분류 결과를 Supabase 에 직접 기록하고, Unity는 Supabase Realtime을 구독하여 재고 변경을 즉시 반영한다.

### 4.3.1 Supabase 구성 요소

| 구성 요소 | 역할 | 비고 |
| --- | --- | --- |
| PostgreSQL | 재고·이력·그리드 랙 마스터 데이터 저장 | Supabase 기본 제공 |
| Realtime | DB 변경 → 구독 클라이언트 즉시 Push | Unity WebSocket 대체 |
| Edge Functions | MQTT 수신 후 그리드 셀 할당 비즈니스 로직 실행 | Deno 런타임, TypeScript |
| Storage | 부품 촬영 이미지 저장 |  |
| Dashboard | 웹 기반 DB 조회·관리 UI | 개발 중 디버깅 활용 |
...
### 4.3.3 MQTT → Supabase 브리지 (Raspberry Pi)

Raspberry Pi에서 경량 Python 스크립트가 MQTT를 구독하고 Supabase Python SDK로 데이터를 직접 기록한다. inventories 테이블 변경 시 Supabase Realtime이 자동으로 Unity에 Push한다.

### 4.3.4 셀 할당 로직 — Edge Function (TypeScript)

부품 분류 완료 후 목적 그리드 셀(X, Y, Z)을 결정하는 로직을 Supabase Edge Function으로 구현한다. 동일 부품 셀 우선 → 빈 셀 순서로 자동 배정한다.
...
- 셔틀 이동 애니메이션: 서보 모터의 실시간 각도값을 수신하여 크랭크 슬라이더 기구학 계산을 통해 X·Y·Z 실제 위치를 산출하고, 물리적 동작과 동기화하여 재생
- UI Overlay: 그리드 셀 클릭 시 부품명·수량·최근 입고 시간 팝업
- 색상 표시: 빈 셀 회색 / 재고 있음 파란색 / 가득 참 주황색

### 4.4.2 Supabase Realtime 연동 (C#)

Unity가 Supabase Realtime을 직접 구독하며 inventories 테이블 변경 이벤트를 수신하여 즉시 3D 씬을 갱신한다.

### 4.4.3 주요 C# 스크립트

| 스크립트 | 역할 |
| --- | --- |
| WarehouseManager.cs | Supabase Realtime 구독, 재고 변경 이벤트 수신 및 씬 동기화 |
| ShuttleController.cs | 서보 모터 각도값 수신 → 크랭크 슬라이더 기구학 계산으로 X·Y·Z 실제 위치 산출 → 셔틀 이동 애니메이션 동기화 재생 |
| GridCell.cs | 그리드 셀 클릭 이벤트, 부품명·수량 팝업 처리 |
| ConveyorAnimator.cs | GT2 타이밍 벨트 이동 애니메이션 (스텝 기반) |
| UIManager.cs | 재고 검색, 필터, 그리드 전체 현황 UI 관리 |
...
• MQTT 브로커(Mosquitto) 설치 및 토픽 구조 확정
• Git 레포지토리 브랜치 전략 적용 및 팀 권한 설정
• ESP32 개발 환경(PlatformIO) 세팅 및 Wi-Fi MQTT 연결 테스트 | UML Diagram, Supabase 테이블 완성, MQTT 송수신 확인, Git 레포 |
| 2단계
하드웨어 MVP | • NEMA17 + A4988 + GT2 컨베이어 정밀 정지 구현 (스텝 카운트 기반)
• TCRT5000 IR 센서 디바운스 및 촬영 트리거 로직
• 4-way 셔틀 크랭크 기구 조립 및 방향 전환 동작 검증
...
• MQTT 브리지 스크립트 → Supabase inventories 테이블 기록 | 부품 자동 분류 성공률 85% 이상, DB 자동 기록 |
| 4단계
Unity 디지털 트윈 | • Blender / CATIA 그리드 랙·셔틀·컨베이어 3D 모델 임포트
• Supabase Realtime C# 구독 (WarehouseManager.cs)
...
| 통신 | MQTT (Mosquitto), WebSocket, REST API |
...
| MG996R 서보 모터 | 4-way 셔틀 Z축 리프트 × 2 + 방향 전환(크랭크 임펠러) × 1 = 총 3개 |
| IR 반사 센서 | TCRT5000 |
...
| 크랭크 기구 방향 전환 불완전 | 높음 | 임펠러 각도 센서 추가, 전환 후 접지 확인 딜레이 적용 |
| 셔틀 위치 오차 (서보 과부하·레일 마찰) | 높음 | 엔드스톱 주기적 재캘리브레이션, 이동 전 토크 이상 감지 루틴 |
| Supabase Realtime 지연 | 중간 | Realtime 채널 재연결 로직 구현, 폴링 폴백 병행 |
...
- **fix/** → `fix/ir-sensor-noise`, `fix/rt-realtime-sync-delay`
...
| docs | 문서 수정 | docs: Supabase 스키마 ERD 추가 |
...
| 컬럼명 | 타입 | 설명 |
| --- | --- | --- |
| `id` | `uuid` (기본) 또는 `bigint` | PK, Supabase 기본은 uuid |
| `created_at` | `timestamptz` | 생성 시각 (Supabase 자동 생성) |
