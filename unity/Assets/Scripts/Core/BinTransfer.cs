...
public class BinTransfer
{
    ...
    public Vector3Int TransferXZ(int fromX, int fromZ, int toX, int toZ)
    {
        ...
        if (!IsInRangeXZ(fromX, fromZ))
            throw new IndexOutOfRangeException($"출발지 셀 ({fromX}, {fromZ})의 좌표가 그리드 범위를 벗어나므로 TransferXZ 불가");
        if (!IsInRangeXZ(toX, toZ))
            throw new IndexOutOfRangeException($"목적지 셀 ({toX}, {toZ})의 좌표가 그리드 범위를 벗어나므로 TransferXZ 불가");

        // 출발지 Bin 검사
        int fromMaxY = FindMaxY(fromX, fromZ);
        if (fromMaxY < 0)
            throw new ArgumentException($"출발지 셀 ({fromX}, {fromZ})에 Bin이 존재하지 않으므로 TransferXZ 불가");
        // 목적지 Bin 가득 찬지 검사
        int toMaxY = FindMaxY(toX, toZ);
        if (toMaxY >= gridSize - 1)
            throw new InvalidOperationException($"목적지 셀 ({toX}, {toZ})에 Bin이 가득 차 있으므로 TransferXZ 불가");
        ...
        if (bin.OnDuty)
            throw new InvalidOperationException($"출발지 셀 ({fromX}, {fromZ})의 Bin이 작업 중이므로 TransferXZ 불가");
        if (binUnder != null && binUnder.OnDuty)
            throw new InvalidOperationException($"목적지 셀 ({toX}, {toZ})의 Bin이 작업 중이므로 TransferXZ 불가");
        if (shuttleBlock != null)
            throw new InvalidOperationException($"목적지 셀 ({toX}, {toZ})에 위치한 Shuttle '{shuttleBlock.Id}'이 방해하므로 TransferXZ 불가");
    ...
    public void MoveShuttleToCell(Shuttle shuttle, int toX, int toZ, int rep = 7)
    {
        ...
        if (FindShuttleByXZ(toX, toZ, shuttle) != null)
            throw new InvalidOperationException($"셀 ({toX}, {toZ})에 다른 셔틀이 이미 존재하므로 MoveShuttleToCell 불가");
 ...
 */
    private Shuttle FindNearShuttle(Vector3Int cell)
    {
        ...
        if (nearest == null)
            throw new InvalidOperationException("사용 가능한 Shuttle이 없음");
