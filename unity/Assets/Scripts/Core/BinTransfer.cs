 ...
 * - BinRegister(Bin, Vector3Int) — Y 무시 (X, 맨 위, Z)에 Bin 쌓아서 등록
...
public class BinTransfer
{
     ...
     * @param  transfer  할당된 셔틀의 IsTransferring을 true로 설정할지 여부 (기본 true)
     * @return 가장 가까운 사용 가능한 Shuttle 인스턴스
     * @throws InvalidOperationException  사용 가능한 Shuttle이 없을 때
     */
    private Shuttle FindNearShuttle(Vector3Int cell, bool transfer = true)
    {
        // 가장 가까운 셔틀 탐색 (맨해튼 거리)
        ...
        // 가장 가까운 셔틀 할당
        nearest.IsTransferring = transfer;
        return nearest;
    }
     ...
     * @param  toCell     목적 셀 좌표
     * @param  rep      재귀 깊이 (기본 7)
     * @throws InvalidOperationException  목적 셀에 다른 셔틀이 이미 존재할 때
     * @throws InvalidOperationException  재귀 횟수 초과 시
     */
    private void MoveShuttleToCell(Shuttle shuttle, Vector3Int toCell, int rep = 7)
    {
        // 목적 셀에 다른 셔틀이 이미 존재하는지 검사
        if (_shuttleList.Exists(s =>
            s != shuttle &&
            s.FromCell.x == toCell.x &&
            s.FromCell.z == toCell.z))
            throw new InvalidOperationException($"셀 ({toCell.x}, {toCell.z})에 다른 셔틀이 이미 존재하므로 MoveShuttleToCell 불가");
        ...
        while (true)
        {
            ...
            Vector3Int targetXZ = new Vector3Int(toCell.x, shuttle.FromCell.y, toCell.z);
     ...
     * @throws InvalidOperationException  리프트가 제자리에 있지 않을 때
