...
public class RevoluteJointComponentPlayModeTest
{
    ...
    {
        // 1. 컴포넌트를 추가하자마자 Awake가 실행되는데, 이때 _objectA가 null이라 에러가 납니다.
        // 이를 방지하기 위해 임시로 비활성화 상태로 생성합니다.
        ...
        // 2. 필드 설정 (확인 결과 필드명은 _objectA, _objectB가 맞습니다!)
        ...
        // 3. 필드 주입이 끝난 후 오브젝트를 활성화하면 이때 안전하게 Awake가 실행됩니다.
    ...
    public IEnumerator Constraints_XAxisRotation_OnlyXRotationAllowed()
    {
        ...
        // 구속이 적용되었는지 확인 (FreezeRotationX 만 해제)
